    ...
    <div className='fixed top-0 w-[100vw] min-h-screen z-[99] bg-black/30 flex items-center justify-center backdrop-blur'>
        ...
        <form onSubmit={onLogin} action="" className='user-popup'>
            ...
            <div className='flex flex-col gap-3'>
                {currState==='Login'?<></>:<input name="name" onChange={onchangeHandler} value={data.name} type="text" placeholder='your name' required className='py-2 border-b w-full bg-black'/>}
               
                <input name="email" onChange={onchangeHandler} value={data.email} type="email" placeholder='email' required className='py-2 border-b w-full bg-black'/>
                <input name="password" onChange={onchangeHandler} value={data.password} type="password" placeholder='password' required className='py-2 border-b w-full bg-black' />
                
                <div className='flex items-center'>
                    <input type="checkbox" required />
                    <p>Lorem ipsum, dolor sit consectetur...</p>
                </div>
                <button type='submit' className='bg-red-500 rounded-full py-2'>{currState==="Sign Up"?"Create Account":"Login"}</button>
